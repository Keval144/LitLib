...
export const DesktopSidebar = ({
  ...
  return (
    <motion.div
      className={cn(
        "hidden h-full shrink-0 px-4 py-4 md:flex md:flex-col",
        "border-r border-[var(--color-border)] bg-[var(--color-card)] shadow-sm",
        className,
      )}
      animate={{
    ...
    >
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div
              className="absolute right-10 top-10 z-50 cursor-pointer text-[var(--color-text)]"
              onClick={() => setOpen(!open)}
            >
              <IconX />
            </div>
...
export const SidebarLink = ({
  ...
  return (
    <Link
      href={link.href}
      className={cn(
        "group/sidebar flex items-center gap-2 rounded-lg px-2 py-2 transition-all",
        "text-[var(--color-text)]",
        "hover:bg-[var(--text-accent-hover)] hover:text-white",
        className,
      )}
      {...props}
    >
